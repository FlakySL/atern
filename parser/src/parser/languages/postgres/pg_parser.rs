...
impl <'src> PP<'src, PgKind, PostgreSQL, Token> for PgParser<'src>{
    ...
    fn parse(&mut self) -> ParseResult<TreeNode<PgKind>, Rich<'src,Token>>{
...
//=====================PARSERS=================//
fn start<'src, I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src,Token>>>
where
...
fn create_table<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>>
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{
    let create = just(Token::Create).to(TreeNode::new_no_term(PgKind::CREATE,vec![]));
    let table = just(Token::Table).to(TreeNode::new_no_term(PgKind::TABLE, vec![]));
    let table_name = select!{
                Token::Identifier(x) => TreeNode::new_term_s(PgKind::NAME, x),
    };
    create
        .then(table)
        .then(table_name)
...
fn column<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>>
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{
    ...
    let column_type = select!{Token::Identifier(type_name) => TreeNode::new_term_s(PgKind::TYPE, type_name),
    };
    ...
    let column_constraints = some_of((
        just(Token::Null)
            ...
            .or(just(Token::Not)
                    .then(just(Token::Null)).to(TreeNode::new_term(PgKind::NOT_NULL, "NOT NULL"))
        ),
        just(Token::Primary)
            .then(just(Token::Key))
...
fn col_foregein_key<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>>
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{
    let core = just(Token::References).ignore_then(
        select!{ Token::Identifier(x) => TreeNode::new_term_s(PgKind::NAME, x),
        }).then(
            select!{ Token::Identifier(y) => TreeNode::new_term_s(PgKind::NAME, y),
    ...
    let r#match = just(Token::Match).ignore_then(
        choice((
            just(Token::Full).to(TreeNode::new_term(PgKind::MATCH_FULL, "MATCH FULL")),
            just(Token::Partial).to(TreeNode::new_term(PgKind::MATCH_PARTIAL, "MATCH PARTIAL")),
            just(Token::Simple).to(TreeNode::new_term(PgKind::MATCH_SIMPLE, "MATCH SIMPLE")),
        ))
    );
...
fn table_constraint<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>>
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{
    todo()
...
fn inner_expr<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>>
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{   recursive(|expr|{
        let operand = choice((
...
fn boolean<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>> + Clone
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{
    let r#true = just(Token::True).to(TreeNode::new_term(PgKind::TRUE, "TRUE"));
    let r#false = just(Token::False).to(TreeNode::new_term(PgKind::FALSE, "FALSE"));
     
    r#true
        .or(r#false)
}

fn identifier<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>> + Clone
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan> 
{
    select!{
        Token::Identifier(x) => TreeNode::new_term_s(PgKind::IDENTIFIER, x),
    }
}

fn number<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>> + Clone
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{
    select!{
        Token::Number(x) => TreeNode::new_term_s(PgKind::NUMBER, x),
    }
}

fn string<'src,I>() -> impl CP<'src, I ,TreeNode<PgKind>, CE<Rich<'src, Token>>> + Clone 
where
    I: ValueInput<'src, Token = Token, Span = SimpleSpan>
{
    select!{
        Token::Text(x) => TreeNode::new_term_s(PgKind::STRING, x),
    }
}
